...
const CleanSymptomChecker = ({ user, onBack }) => {
  const [messages, setMessages] = useState([
    {
      id: 1,
      type: 'assistant',
      message: `Hello! I'm ARYA, your Personal Health Assistant. I'm here to listen and help with any health concerns you have. What's on your mind today?`,
      timestamp: new Date()
    }
  ]);
  ...
  const addMessage = (type, message, urgencyLevel = null, messageId = null) => {
    ...
    // Play subtle notification sound for assistant messages
    if (type === 'assistant') {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Call ENHANCED hybrid clinical system API (universal orchestrator - works for all symptoms)
      const response = await fetch(`${BACKEND_URL}/api/enhanced-hybrid/chat`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
